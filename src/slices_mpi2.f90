...
!
      ...
      if (lfirst_call) then
        call read_snaptime(file,tvid,nvid,dvid,t,lfirst_call)
        lfirst_call=.false.
      endif
!
          ...
          if (slices%index==0) then    ! If this wasn't a multi slice...
            ...
            if (associated(slices%xy2)) &
              call wslice(path//trim(slices%name)//trim(sindex)//'.xy2', &
                          slices%xy2, z(iz2_loc),nx,ny,nxgrid,nygrid)
            if (associated(slices%xy3).and.lwrite_slice_xy3) &
              call wslice(path//trim(slices%name)//trim(sindex)//'.xy3', &
                          slices%xy3, z(iz3_loc),nx,ny,nxgrid,nygrid)
            if (associated(slices%xy4).and.lwrite_slice_xy4) &
              call wslice(path//trim(slices%name)//trim(sindex)//'.xy4', &
                          slices%xy4, z(iz4_loc),nx,ny,nxgrid,nygrid)
          endif
        else
          if (slices%index==0) then    ! If this wasn't a multi slice...
...
!
    endsubroutine wvid
!***********************************************************************
    subroutine wslice(filename,a,pos,ndim1,ndim2,nglob1,nglob2)
!
...
!  22-sep-07/axel: changed Xy to xy2, to be compatible with Mac
!
      integer :: ndim1,ndim2,nglob1,nglob2
      character (len=*) :: filename
      real, dimension (ndim1,ndim2) :: a
      real, intent(in) :: pos
      integer :: iostat
!
      integer :: MPI_COMM_SLICE
      integer :: local_type, global_type, mpi_err
      integer, dimension(MPI_STATUS_SIZE) :: status
      integer (kind=MPI_OFFSET_KIND) :: displacement
      integer, parameter :: io_dims=2, order=MPI_ORDER_FORTRAN, io_info=MPI_INFO_NULL
      integer, dimension(io_dims) :: local_size, local_start, global_size, global_start, subsize
      integer :: fh
!
!  Select the appropriate MPI Communicator
!  for the slice plane and set globale starts
!
      displacement = 0
      if (index(filename,'xy') > 0) then
        MPI_COMM_SLICE = MPI_COMM_XYPLANE
        global_start = (/ ipx*nx, ipy*ny /)
      elseif (index(filename,'yz') > 0) then
        MPI_COMM_SLICE = MPI_COMM_YZPLANE
        global_start = (/ ipy*ny, ipz*nz /)
      elseif (index(filename,'xz') > 0) then
        MPI_COMM_SLICE = MPI_COMM_XZPLANE
        global_start = (/ ipx*nx, ipz*nz /)
      endif
!
...
!  Define array sizes
!
        local_size  = (/ ndim1,  ndim2 /)
        subsize     = (/ ndim1,  ndim2 /)
        global_size = (/ nglob1, nglob2 /)
        local_start = (/ 0, 0 /)
!
...
!
! Open MPI file in all processes in the MPI_COMM_SLICE communicator and set
! displacement to end of file
!
        call MPI_FILE_OPEN (MPI_COMM_SLICE, filename, MPI_MODE_WRONLY + &
                            MPI_MODE_CREATE + MPI_MODE_APPEND, io_info, fh, mpi_err)
        call MPI_FILE_GET_SIZE (fh, displacement, mpi_err)
!
! Setting file view and write raw binary data, ie. 'native'.
!
        call MPI_FILE_SET_VIEW (fh, displacement, mpi_precision, global_type, &
                                'native', io_info, mpi_err)
!
        call MPI_FILE_WRITE_ALL (fh, a, 1, local_type, status, mpi_err)
!
...
!
        write(1,IOSTAT=iostat) tslice,pos
        if (outlog(iostat,'write tslice,pos')) return
!
        close(1,IOSTAT=iostat)
        if (outlog(iostat,'close')) continue
      endif
...
!
      if (lroot) then
        open(lun,file=trim(directory_snap)//'/slice_position.dat',STATUS='unknown',IOSTAT=iostat)
!
...
!
        if (outlog(iostat,'open',trim(directory_snap)//'/slice_position.dat')) return
!
